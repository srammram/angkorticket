...
io.sockets.on('connection', function (socket){
	...
	schedule.scheduleJob('0 * * * * *', function(){
		...
		pool.query("SELECT * FROM kapp_rides WHERE ride_type = 2 AND emit_status = 0 AND booking_timing >= DATE_SUB(NOW(), INTERVAL 1 MINUTE)   ", function (error, results, fields){
			if(error){
			...
			if(results.length == 0){
				...
				pool.query("SELECT  d.id, d.mobile, d.country_code, d.oauth_token, dcs.current_latitude latitude, dcs.current_longitude longitude, dcs.mode, d.first_name, up.last_name, up.photo as driver_photo, t.name as taxi_name, t.model, t.number, t.type, t.photo as taxi_photo,  tt.name type_name, tt.image, tt.image_hover, tt.mapcar type_image,  g.name as group_name,   ( 6371 * acos( cos( radians("+results[0].start_lat+") ) * cos( radians( dcs.current_latitude ) ) * cos( radians( dcs.current_longitude ) - radians("+results[0].start_lng+") ) + sin( radians("+results[0].start_lat+") ) * sin( radians( dcs.current_latitude ) ) ) ) AS distance FROM kapp_users  AS d LEFT JOIN kapp_driver_current_status AS dcs ON dcs.driver_id = d.id  LEFT JOIN kapp_user_profile AS up ON up.user_id = d.id  LEFT JOIN kapp_taxi AS t ON t.id = dcs.taxi_id  LEFT JOIN kapp_taxi_type AS tt ON tt.id = t.type LEFT JOIN kapp_groups AS g ON g.id = d.group_id WHERE dcs.mode = 1 AND dcs.is_connected = 1 AND dcs.allocated_status = 1  GROUP BY d.id HAVING distance <= 100 ORDER BY distance ASC", function (error, dresults, fields){
						...
						pool.query("SELECT * FROM kapp_user_socket WHERE user_id = '"+dresults[0].id+"'   ", function (error, uresults, fields){
							if(uresults.length == 0){
								...
								if(uresults[0].socket_id != null){
									...
									var users = 
									{ "booked_type_text": booked_type_text, "payment_id": results[0].payment_id, "payment_name": results[0].payment_name, "distance_km":results[0].distance_km, "distance_price":results[0].distance_price, "customer_support": "0987654321",  "pick_up": results[0].start, "from_latitude": results[0].start_lat,
									...
									"driver_id": dresults[0].id, "driver_oauth_token": dresults[0].oauth_token, "socket_id": uresults[0].socket_id}
									;
									console.log(users);
									
										
										//socket.to(results[0].socket_id).emit('vendor_tracking_driver', res);
										socket.to(uresults[0].socket_id).emit('driver_available_check', users);
	...
	socket.on('server_ride_change', function(data){
		console.log('Socket Ride Change');
		//console.log('Server Ride Cancel');
		console.log(data);
		socket.to(data.socket_id).emit('driver_ride_change', data );
		//io.sockets.emit('admin_drivers_cancel', data);
	});
				...
				if(error){
					...
					if(results.length == 0){
						...
						if(results[0].ride_id != null){
							...
							var change_type = results[0].change_type;	
							var middle = results[0].middle;	
							var middle_lat = results[0].middle_lat;	
							var middle_lng = results[0].middle_lng;	
							...
							socket.to(results[0].socket_id).emit('get_driver_location', { ride_id: cus_ride_id, lat: cus_lat, lng: cus_lng, pickup_lat: cus_pickup_lat, pickup_lng: cus_pickup_lng, drop_lat: cus_drop_lat, drop_lng: cus_drop_lng, actual_lat: cus_actual_lat, actual_lng: cus_actual_lng, change_type:change_type, middle:middle, middle_lat:middle_lat, middle_lng:middle_lng });
