...
class Auth_model extends CI_Model
{
	...
	public function random_check(){
		...
		while ($continue) {
			$v = mt_rand(1000, 9999);
			$query = $this->db->select('user_number')
			->where('user_number', $val)
			->limit(1)
			->get($this->tables['users']);
			if($query->num_rows() != 1)
				$continue = false;
    ...
    public function login($identity, $password, $country_code, $group_id, $remember = FALSE)
    {
        ...
        if ($query->num_rows() == 1) {
            ...
            if ($user->password === md5($password)) {
                ...
                //if($this->site->isloggeddIn($identity)){
                //    $this->session->set_flashdata('error', lang('someone has logged in'));
                ...
                $ldata['session_id'] = $user->session_id;
                $ldata['email'] = $user->email;
                $ldata['username'] = $user->username;
				$ldata['mobile'] = $user->mobile;
				$ldata['first_name'] = $user->first_name;
				$ldata['last_name'] = $user->last_name;
    ...
    public function increase_login_attempts($identity)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $ip_address = $this->_prepare_ip($this->input->ip_address());
            return $this->db->insert($this->tables['login_attempts'], array('ip_address' => $ip_address, 'type' => 'admin', 'login' => $identity, 'time' => time()));
        }
        return FALSE;
    }
